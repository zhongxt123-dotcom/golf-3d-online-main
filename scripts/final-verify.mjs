...
console.log(`Total courses: ${golfLocations.length}`);

// 1. Province-coordinate mismatch check
...
golfLocations.forEach((loc, i) => {
  ...
  if (!bounds) { coordIssues++; console.log(`  NO BOUNDS: #${i} ${loc.name} -> ${p}/${c}`); return; }
  if (loc.lat < bounds.minLat || loc.lat > bounds.maxLat || loc.lng < bounds.minLng || loc.lng > bounds.maxLng) {
    coordIssues++;
    console.log(`  BAD: #${i} ${loc.name} -> ${p}/${c} (${loc.lat},${loc.lng})`);
  }
});
console.log(`  Results: ${coordIssues} issues`);

// 2. Mission Hills Haikou cluster check
console.log(`\n2. Mission Hills Haikou Cluster:`);
const mhHk = golfLocations.filter(loc => loc.name.includes("海口观澜湖"));
mhHk.forEach(loc => {
  console.log(`  ${loc.name}: ${loc.lat}, ${loc.lng} (${loc.province}/${loc.city})`);
});

// 3. Mission Hills Shenzhen cluster check
console.log(`\n3. Mission Hills Shenzhen/Dongguan Cluster:`);
const mhSz = golfLocations.filter(loc => loc.name.includes("深圳观澜湖") || loc.name.includes("东莞观澜湖"));
mhSz.forEach(loc => {
  console.log(`  ${loc.name}: ${loc.lat}, ${loc.lng} (${loc.province}/${loc.city})`);
});

// 4. Province distribution check
console.log(`\n4. Province Distribution:`);
const provCount = {};
golfLocations.forEach(loc => {
  const p = loc.province || "N/A";
  provCount[p] = (provCount[p] || 0) + 1;
});
Object.entries(provCount).sort((a,b) => b[1]-a[1]).forEach(([prov, count]) => {
  console.log(`  ${prov}: ${count} courses`);
});

// 5. Courses with "N/A" province (unmatched)
const unmatched = golfLocations.filter(loc => !loc.province || loc.province === "中国");
console.log(`\n5. Unmatched Courses (no province): ${unmatched.length}`);
unmatched.forEach(loc => console.log(`  ${loc.name}: ${loc.lat}, ${loc.lng}`));
